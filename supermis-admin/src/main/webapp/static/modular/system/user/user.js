...
MgrUser.openAddMgr = function () {
    if(MgrUser.roleid>1){
        ...
        sessionStorage.setItem("roleid", MgrUser.roleid);
        this.layerIndex = index;
    }else{
        Feng.info("请先在权限范围内选中具体角色！");
    }
...
MgrUser.search2 = function () {
    var queryData2 = {};

    queryData2['roleid'] = MgrUser.roleid;
    queryData2['name'] = $("#name").val();
    queryData2['beginTime'] = $("#beginTime").val();
    queryData2['endTime'] = $("#endTime").val();
 // alert(JSON.stringify(queryData2));
    MgrUser.table.refresh({query: queryData2});
}
...
MgrUser.onClicknRole = function (e, treeId, treeNode) {
    MgrUser.roleid = treeNode.id;
    MgrUser.search2();
};
...
$(function () {
    ...
    // var table = new BSTable("managerTable", "/mgr/listRoles", defaultColunms);

    table.setPaginationType("client");
    MgrUser.table = table.init();
    var ztree = new $ZTree("deptTree", "/role/tree");
    // var ztree = new $ZTree("deptTree", "/dept/tree");
    // ztree.bindOnClick(MgrUser.onClickDept);
    ztree.bindOnClick(MgrUser.onClicknRole);
    ztree.init();
